...
from worker.celery import app, BROKER_CONF, KAFKA_TOPIC_CONF
...
def transport_wmi_metric_to_kafka_with_fields(hostname, username, password, classname, fields):
	...
	if res is None:
		...
		p.produce(KAFKA_TOPIC_CONF, json.dumps(res).encode('utf-8'), callback=delivery_report, timestamp=int(timestamp))
		p.flush()
